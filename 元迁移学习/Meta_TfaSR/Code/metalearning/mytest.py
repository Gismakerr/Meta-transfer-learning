...
class Test(object):
    def __init__(self, model_path, save_path,kernel, scale, method_num, num_of_adaptation):
        ...
        # 比较结果的
        self.model = Models.MSRResNet1(1, 1) # 元模型
        self.model_tfasr = Models.MSRResNet1(1, 1)  # tfasr
    ...
    def __call__(self, test_path, train_path):
        ...
        self.model_tfasr.load_state_dict(torch.load(r"H:\generator_final_100.pth", map_location=torch.device("cuda:0")))
    ...
    def train(self):
        #self.hr_father,_,_ = imread(r"H:\meta_train_data\gt\desert1_0_1.tif")
        #self.lr_son,_,_ = imread(r"H:\meta_train_data\img\desert1_0_1.tif")
        ...
        for i in range(self.iter):
            train_list = random.sample(self.train_path, 8)
            ...
            for hr_path in (train_list):
                ...
                icesat = r"H:\\meta_train_data\\gt\\desert\\"+hr_path.split("\\")[-1].split("_")[0]+"_icesat_"+hr_path.split("\\")[-1].split("_")[1]+"_"+hr_path.split("\\")[-1].split("_")[2]
                icesat = read_tif(icesat)[0][5]
                lr_son = Resize(hr_father, int(self.gt.shape[0] / self.scale), int(self.gt.shape[1] / self.scale), "LINEAR")

                self.train_output = self.forward_backward_pass((lr_son,icesat), hr_father)
            # Display information
    ...
    def quick_test(self):
        for i,gt_path in enumerate(self.test_path):
            ...
            self.icesat = read_tif(r"H:\\meta_train_data\\gt\\desert\\"+gt_path.split("\\")[-1].split("_")[0]+"_icesat_"+gt_path.split("\\")[-1].split("_")[1]+"_"+gt_path.split("\\")[-1].split("_")[2])[0][5]
            ...
            self.mse_tfasr = self.mse_tfasr + [np.mean((self.gt - self.sr1)**2)]
    ...
    def forward_pass_tfasr(self, input, output_shape=None):
        ...
        ILR = torch.tensor(self.Resize(input[0], output_shape[0], output_shape[1], "NEAREST"), dtype=torch.float32)   #.permute(2, 0, 1).to("cuda:0")
        ...
        output = self.model_tfasr(ILR, icesat)
    ...
    def forward_pass(self, input, output_shape=None):
        ...
        output = self.model(ILR, icesat)
        output = output.detach().cpu().numpy()######
        return np.squeeze(output)
        #return np.clip(np.squeeze(output), 0., 1.)
    
    def GetNormalData(self, data_temp):
    ...
    def forward_backward_pass(self, input, hr_father):
        ...
        max, min = torch.max(ILR), torch.min(ILR)
        ILR_norm = self.GetNormalData(ILR)

        train_output = self.model(ILR_norm, icesat)
        self.loss = self.loss_fn(train_output, self.GetNormalData(HR))
